...
| `test_client_comparison_section_present_and_populated` | `## Client comparison` sits between `## Proposed gas parameters` and `## Warnings`; its header row matches the documented column order; rows carry worst-then-second-worst clients with a signed diff and percent. |
