...
Plan rules pinned: a spec's `overhead_baseline_param` pairs each fixture where that param is `"False"` against its `"True"` counterpart (same client, same every other parsed param) and fits `target_coef` on the runtime delta instead of raw runtime; the pairing step runs before the §2.3 opcount invariant (every `True` row has `opcount == 0`); an unmatched `False` row is a `ConfigError`; the pairing transform diffs every opcode-count column, not just runtime, so `compute_glue_opcodes_by_test` runs on baseline-paired specs against that same delta instead of being skipped — a contaminant identical between variants deltas to a constant and fails detection on its own (no double-subtraction), while one that scales with the target's own opcount survives the diff and is still detected and priced.
...
| `test_baseline_pair_cancels_matching_contaminant_out_of_glue_detection` | Same contaminated fixtures, glue enabled. Asserts `glue_opcodes_by_test.csv` has zero rows for the baseline-paired spec's `test_name`, even though the contaminant strongly correlates with the target's raw opcount — its count is identical in both variants, so it deltas to a constant and fails the correlation threshold on its own. |
| `test_baseline_pair_still_prices_non_cancelling_glue_opcode` | A priced glue opcode (`GAS`, with a driver fixture via `make_glue_driver_fixtures()`) planted one-for-one with the target's own opcount on the `False` side only (mirrors the CALL-family calling-convention setup the `True` baseline also drops). Asserts: `glue_opcodes_by_test.csv` flags `GAS` for the baseline-paired spec's `test_name`; the raw (pre-adjustment) `results.csv` `target_coef_runtime_ms` still carries the full contamination (pairing alone doesn't cancel it); `new_gas_all_params.csv`'s `glue_adjustment` is nonzero and the post-adjustment `runtime_ms` recovers the clean planted slope. |
