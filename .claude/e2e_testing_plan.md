...
- **Fixture builders**: `make_block_limit_fixtures(...)`, `cross_product_fixtures(...)`, `make_glue_driver_fixtures(...)`. The last sources `(test_name, target_opcode)` pairs from `evm_gasfit.glue.required.REQUIRED_GLUE_TESTS` so the suite tracks the implementation's priced-glue set.
...
Plan rules pinned: glue off by default; the priced opcode set is sourced from `evm_gasfit.glue.required.REQUIRED_GLUE_TESTS`; the `(test_name, target_opcode)` required-inputs check.
...
| `test_glue_enabled_writes_files_and_adjusts_slope` | Imports `REQUIRED_GLUE_TESTS` and derives the priced-opcode set from it. Synthesizes main-model fixtures with a non-trivial `POP` background plus driver fixtures for every required `(test_name, target_opcode)` pair via `make_glue_driver_fixtures()`. Enables glue with p-value threshold 0.05. Asserts: glue CSVs + glue markdown exist; `glue_results.csv` columns ⊇ `{client_name, glue_opcode, nobs, glue_runtime_ms, p_value, rsquared}`; the `glue_opcode` set equals the priced set; `new_gas_all_params.csv` row for `OPCODE_ADD` has `glue_adjustment > 0` and `new_gas_decimal = anchor_rate · runtime_ms / 1000` still holds with the post-adjustment `runtime_ms`. |
