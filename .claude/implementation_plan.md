...
| `runtime_estimation_autogenerated_report.md` | yes | per-spec regression report. Opens with a `## Contents` bulleted list (one bullet per `test_name`, anchors GFM-compatible). Each test gets a `## <test_name>` section; within it, each `(target_opcode, model_by-combo)` has a headline `### <opcode>` (with optional `— combo <combo>` suffix), the cross-client headline metrics table, and one `<details>` block per client with summary `"<client> — NNLS regression summary"` wrapping the NNLS summary plus, when `output.plots: true`, the three per-fit plot embeds (regression, bootstrap, diagnostics). |
...
| `new_gas_proposal.md` | yes | final proposal, diff vs. patched fork values + `new_params` integer defaults (§4.6) with "no prior default" sentinel for `new_params` entries declared as `null`. Anchor rate is rendered as `<N> Mgas/s` (3 significant figures over `anchor_rate / 1e6`) in the run-metadata line. Sections, in order: `## Proposed gas parameters` diff table with columns `gas_param \| current_gas \| proposed_gas \| diff \| diff %` (column headers are title-cased — `Gas param`, `Current gas`, `Proposed gas`, `Diff`, `Diff %`) and fitted rows only; `## Client comparison` (one combined table with one row per gas_param: worst client + value, second-worst client + value, gas diff, % diff — gas params fitted by a single client are omitted; the heatmap plot is embedded at the end of this section); `## Warnings` containing `### Unresolved (no fit)` (always present — `_None._` body when empty; lists no-fit / None-derived rows), `### Missing glue opcodes` (when populated; §4.4 missing-glue warnings), and `### Other` (when populated; config-load warnings from §2.5 and anything else); `## Poor-fit selections`. |
...
The proposal heatmap is **row-normalized to a per-row best-to-worst gradient**: each `gas_param` row is colored on its own [min, max] scale so within-row client differences are not flattened by between-row order-of-magnitude gaps. Encoding: `(value - row_min) / (row_max - row_min)` rendered with `RdYlGn_r` — the cheapest (best) client in each row is green and the worst is red, with a linear gradient between them; single-client rows (and rows where every client has the same value) are neutral. Absolute gas values remain in the cell annotations. Embedded in the `## Client comparison` section of `new_gas_proposal.md`. The previous per-client bar plot (`figs/proposal/by_client.png`) is **not produced** — the heatmap subsumes it.
