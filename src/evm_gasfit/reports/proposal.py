...
def _render_weak_losers_table(
    ...
    """Condensed rendering for ``### Other weak candidates``.

    One row per (gas_param, test_name, target_opcode, model_coef_name) — the
    failing clients are collapsed into a single ``Clients`` cell with each
    client's failure label in parens. Per-fit metrics (``runtime_ms``,
    ``pvalue``, ``rsquared``) are intentionally dropped; readers can
    cross-reference ``runtime_estimation_autogenerated_report.md`` when they
    need the numbers behind a particular fit.
    """
    ...
    for _, row in rows_df.iterrows():
        key = (
            ...
            str(row["model_coef_name"]),
        )
        label = _poor_fit_failure_label(
            row["pvalue"], row["rsquared"], pv_thresh, r2_thresh
        )
        grouped.setdefault(key, []).append(f"`{row['client_name']}` ({label})")

    lines = [
        "| Gas param | Test | Target opcode | Coef | Clients |",
        "| --- | --- | --- | --- | --- |",
    ]
    for (gas_param, test_name, target_opcode, coef), clients in grouped.items():
        lines.append(
            f"| `{gas_param}` | `{test_name}` | `{target_opcode}` | "
            f"`{coef}` | {', '.join(clients)} |"
        )
    return lines
